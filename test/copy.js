...
// exec( 'touch from/site{a,b,c}/some/config.txt' );
// exec( 'touch from/site{a,b,c}/some/long/test{a,b,c}.txt' );
// exec( 'touch from/site{a,b,c}/some/long/config.txt' );
...
exec( 'touch from/sitea/some/config.txt from/siteb/some/config.txt from/sitec/some/config.txt' );
...
exec( 'touch from/sitea/some/long/config.txt from/siteb/some/long/config.txt from/sitec/some/long/config.txt' );
...
describe( 'Teste de cópias dos arquivos', function(){
	describe( 'Copier module', function(){
		...
		it( 'Deve procurar o arquivo recursivamente e parar após copiar o primeiro arquivo encontrado', function( done ){
			copier.removeAllListeners( 'copySuccess' );
			copier.removeAllListeners( 'copyError' );
			...
			copier.copyFileRecursive( 'from/sitea/some/long/path1/config.txt', 'to/sitea/some/long/path1/config.txt' );

		});

		it( 'Deve procurar o arquivo recursivamente e continuar copiando até passar por todos as pastas', function( done ){
			copier.removeAllListeners( 'copySuccess' );
			copier.removeAllListeners( 'copyError' );

			copier.on( 'recursiveCopySuccess',function( data ){
			...
			copier.on( 'recursiveCopyError',function( data ){
				throw( 'Erro ao copiar o arquivo' );
				done();
			});

			copier.copyFileRecursive( 'from/sitea/some/long/config.txt', 'to/sitea/some/long/config.txt', true );
